...
class MLM(Task):
    ...
    def sop_task(self, document: PersonDocument):
        p = np.random.rand(1)
        if p < 0.05:
            document.lifeseq.reverse()
            # document.monthseq.reverse()
            targ_sop = 1
        elif p > 0.95:
            shuffle(document.lifeseq)
            # combined = list(zip(document.lifeseq, document.monthseq))
            # shuffle(combined)
            # document.lifeseq, document.monthseq = zip(*combined)
            targ_sop = 2
        else:
            targ_sop = 0
