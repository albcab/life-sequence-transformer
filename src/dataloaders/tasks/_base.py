...
class Task:
    ...
    def get_document(self, person_sentences: pd.DataFrame) -> PersonDocument:
        ...
        years, months = [], []
        for sm, sy, em, ey in zip(start_month, start_year, end_month, end_year):
            if sy > threshold_year:
                ...
                sentences = sentences[:-1]
                continue
            if ey == threshold_year:
                em = min(em, threshold_month)
            elif ey > threshold_year:
                ey = threshold_year
                em = threshold_month
            years.append([year for year in range(sy, ey + 1)])
            if ey - sy == 0:
                months.append([(sm, em - sm + 1)])
            else:
                months.append([(sm, 12 - sm + 1)] + [(1, 12)] * (ey - sy - 1) + [(1, em)])
        ...
        assert len(years) > 0, f"No life events for {person_id}"
        ...
        for year, month, sentence in zip(years, months, sentences):
            for y, (m, d) in zip(year, month): #shuffling of sentence could happen here
                lifeseq[y - first_year].append([f"MONTH_{m}"] + sentence + [f"DUR_{d}"])
                monthseq[y - first_year].append(m)
