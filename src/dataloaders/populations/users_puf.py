...
@dataclass
class UserMonthPopulation(Population):
    """
    ...
    name: str = "users"
    input_csv: Path = DATA_ROOT / "rawdata" / "users_puf.csv"
    earliest_birthday: str = "01-01-1950"

    seed: int = 42
    train_val_test: Tuple[float, float, float] = (0.7, 0.15, 0.15) #(0.8, 0.2, 0.)
    ...
    def population(self) -> pd.DataFrame:
        ...
        result = dd.read_csv(
            ...
            subset=["BIRTHDAY_MONTH"]
        ).assign(
            AREA=lambda x: ("A" + x.AREA_BIRTH).fillna("[UNK]"),
