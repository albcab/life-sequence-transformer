...
class CorpusVocabulary(Vocabulary):
    ...
    general_tokens: List[str] = field(
        default_factory=lambda: [
            ...
            # "[CLS]",
            # "[SEP]",
            "[BOL]", #begining-of-lifeseq for decoder
            "[EOL]", #end-of-lifeseq for decoder
            "[EOY]", #end-of-year for decoder
            "[MASK]",
    ...
    def token_counts(self, source: TokenSource) -> Dict[str, Dict[str, int]]:
        ...
        tokenized = self.corpus.tokenized_and_transformed(source).reset_index().set_index("USER_ID", sort=False, sorted=True).loc[lambda x: x.index.isin(ids)]
        # tokenized = source.tokenized()[lambda x: x.index.isin(ids)]
